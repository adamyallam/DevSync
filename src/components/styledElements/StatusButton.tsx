...
const StatusButton = ({ status }: StatusButtonProps) => {
  ...
  const statusConfig = {
    ...
    OnHold: { label: 'On Hold', textColor: 'text-[#02BDFF]', bgColor: 'bg-[#1A5AB5]', secondBgColor: 'bg-[#02BDFF]', borderColor: 'border-t-[#1A5AB5]' },
    Complete: { label: 'Complete', textColor: 'text-green-200', bgColor: 'bg-green-700', secondBgColor: '', borderColor: 'border-t-green-700' }
  };
  ...
  const changeStatus = async (newStatus: string) => {
    ...
    try {
      ...
      if (res.ok) {
        setNewStatusKey(newStatus as StatusKey)
