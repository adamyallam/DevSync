...
const StatusButton = ({ status, model, project, task }: Props) => {
  ...
  return (
    <div className={`${model === 'task' ? 'relative' : ''} w-full flex flex-col justify-center`}>
      ...
      {statusChangeOpen && (
        <div ref={menuRef} className={`absolute cursor-default hover:cursor-pointer z-50 ${model === 'task' ? 'top-full w-full mt-1 pb-4' : 'w-[15%] h-[30%] top-24'} border-[4px] border-undertone ${statusDetails.borderColor} rounded-md bg-primary overflow-auto`}>
