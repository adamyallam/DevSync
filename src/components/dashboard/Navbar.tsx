...
export const Navbar = () => {
  ...
  return (
      <div className='fixed top-0 left-0 right-0 z-20 bg-[#212526]'>
        {/* Topbar */}
        <div className='flex items-center justify-between h-12'>
          <div className='flex items-center gap-3'>
            <button className='pl-3 text-[#D0E8E8] hover:text-[#89979E] hover:scale-110 transition-transform' onClick={() => toggleSidebar(!isSidebarOpen)}>
        ...
        {/* Sidebar */}
        <div className={`flex flex-col fixed left-0 bottom-0 top-12 bg-[#212526] text-[#D0E8E8] w-60 transition-transform duration-300 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} overflow-x-hidden overflow-y-auto`}>
          <div className='bg-[#212526] border-t-2 border-b-2 border-[#3A4042] pt-2 pb-2'>
            <Link href='/dashboard/home' className={`flex items-center h-8 ${applySidebarClass('dashboard/home')}`}>
              <Home size={20} className='text-[#D0E8E8]' strokeWidth={1.5} />
              <span className='ml-1 text-sm text-[#D0E8E8]'>Home</span>
            </Link>
          ...
          <div>
            ...
            <div className='pb-3 pt-3'>
              <h1 className='font-semibold pl-2 ml-6 text-[#D0E8E8]'>Projects</h1>
              {projects?.length ? (
                <div>
              ...
              <div className='border-t-2 border-[#3A4042] mt-1 ml-7 pl-1 w-3/4'>
                <button className={`flex items-center justify-center rounded-sm mt-1 w-3.5 h-3.5 hover:scale-125 transition-transform`} onClick={toggleProjectsTab}>
                  {isProjectsCollapsed ? <ChevronDown size={16} strokeWidth={4} className='text-[#D0E8E8] hover:text-[#89979E]'/> : <ChevronUp size={16} strokeWidth={4} className='text-[#D0E8E8]'/>}
                </button>
              </div>
            </div>
          </div>
          <div className='w-full flex flex-col mt-auto items-center border-t-2 border-[#3A4042] pb-4'>
            <button onClick={() => toggleCreateProjectForm(!isCreateProjectFormOpen)} className='text-[#D0E8E8] border-2 border-gray-300 p-2 w-10/12 mt-4 hover:font-semibold hover:text-[#89979E] hover:border-[#89979E] hover:scale-105 transition-transform'>Create Project</button>
            <div className='flex mt-2 gap-2'>
              <button><Instagram strokeWidth={1.5} className='h-6 text-[#D0E8E8] hover:text-[#89979E] hover:scale-105 transition-transform'/></button>
              <button><Twitter strokeWidth={1.5} className='h-6 text-[#D0E8E8] hover:text-[#89979E] hover:scale-105 transition-transform'/></button>
              <button><Linkedin strokeWidth={1.5} className='h-6 text-[#D0E8E8] hover:text-[#89979E] hover:scale-105 transition-transform'/></button>
            </div>
          </div>
        </div>
