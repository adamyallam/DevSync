  ...
  return (
    <div onContextMenu={toggleContextMenu} className={`grid grid-cols-10 grid-rows-1 border-b-2 border-undertone gap-2 h-10 hover:bg-selected group`}>
      <div className={`flex col-span-4 border-r-2 border-undertone`}>
        ...
        <AutoResizingInput focusInput={focusTask} initialWidth={150} maxGrowthWidth={isSidebarOpen ? 444 : 535} placeholder="Name" initialText={taskName} textStyles="text-sm" onConfirmChange={(newName) => updateTaskDatabase(task, project, 'name', newName)} />
      </div>
